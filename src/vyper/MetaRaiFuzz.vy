...
@notice 3pool metapool implementation contract with external rates (RAI)
...
# Fuzz: changed these so they are dinamically assigned
...
@external
def __init__():
    # changed for fuzzing (allowign this contract to be a pool)
    self.fee = 0
...
def initialize(
    ...
    self.base_coins = [_dai , _usdc, _usdt]

    A: uint256 = _A * A_PRECISION
    self.coins = [_coin, _base_pool_token]
...
def get_virtual_price() -> uint256:
    ...
    """
    amp: uint256 = self._A()
    rates: uint256[N_COINS] = [self._get_scaled_redemption_price(), Curve(self.base_pool).get_virtual_price()]
